...
public function index(Request $request)
{
    ...
    if ($type) {
        $dasarCourses = (clone $baseQuery)->where('courseLevel', 'dasar')->get();
        $menengahCourses = (clone $baseQuery)->where('courseLevel', 'menengah')->get();
        $lanjutanCourses = (clone $baseQuery)->where('courseLevel', 'lanjutan')->get();

        $courses = collect(); 
    } else {
        $dasarCourses = (clone $baseQuery)->where('courseLevel', 'dasar')->take(4)->get();
        $menengahCourses = (clone $baseQuery)->where('courseLevel', 'menengah')->take(4)->get();
        $lanjutanCourses = (clone $baseQuery)->where('courseLevel', 'lanjutan')->take(4)->get();
